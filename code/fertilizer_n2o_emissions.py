# -*- coding: utf-8 -*-
"""
Created on Thu Dec  4 09:46:40 2025
...
#%%载入XY
n2odf = pd.read_excel(r'./n2oXY.xlsx')
X = n2odf.drop(columns=['Title', 'Author-Year', 'Lat', 'Lon','UNIQUE', '合并', 'EF','Title', 'Fertilizer'])
Y = n2odf['EF']

#%% ====== 1. 载入参数和 multipliers ======
grid = joblib.load('./RF_grid_V675.pkl')
best_params = grid.best_params_

mult_file = "./CV_FIANL.xlsx"
mult_sheet = "A_multipliers_N2O"
...
X_col = ['Tmp', 'Prec', 'BD', 'Clay', 'SOC', 'TN', 'pH', 'STP', 
         'Nrate', 'UOA', 'manure', 'Others', 'compound', 'DPM', 
         'SBC', 'wheat', 'maize', 'rice', 'others']
...
dfs = ['rice2000', 'wheat2000', 'maize2000', 'rice2001', 'wheat2001', 'maize2001',
       ...
       'rice2022', 'wheat2022', 'maize2022',  'other2000','other2001', 'other2002', 
       'other2003', 'other2004', 'other2005', 'other2006', 'other2007', 'other2008', 
       'other2009', 'other2010', 'other2011', 'other2012', 'other2013','other2014', 
       'other2015', 'other2016', 'other2017', 'other2018', 'other2019', 'other2020', 'other2021', 'other2022']

n_boot = 100
n_mc   = 1000   # Monte Carlo = multiplier 1000 次

# 输出目录（可根据实际情况修改）
os.makedirs("./results_n2o/", exist_ok=True)
...
for df_name in dfs:
    ...
    # ---- 读取数据 ----
    ds = pd.read_excel(f'./prepare_n2o/{df_name}.xlsx')
    ...
    EM_sim = EF_sim * AL_sim * 1.5714 / 100000    # n_rows × 1000
...
for year in sorted(df_ef["year"].unique()):
    for crop in ["rice", "wheat", "maize", "other"]:
...
ef_summary_df.to_csv("./results_n2o/EF_uncertainty_national.csv", index=False)
...
df_city_wide.to_excel("./results_n2o/all_city_MC.xlsx", index=False)
